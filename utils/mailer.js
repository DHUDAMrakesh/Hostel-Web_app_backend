  ...
  const html = `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #f8faff; border-radius: 16px; overflow: hidden; border: 1px solid #e2e8f0;">
      ...
      <div style="padding: 32px 36px;">
        ...
        <p style="margin: 0; font-size: 13px; color: #64748b; line-height: 1.6; border-top: 1px solid #f1f5f9; padding-top: 20px;">
...
/**
 * Send a verification code to a student.
 * @param {string} to   - Student's email
 * @param {string} name - Student's name
 * @param {string} otp  - 6-digit OTP code
 */
async function sendOtpEmail(to, name, otp) {
  console.log(`Attempting to send OTP email to ${to} for ${name} with OTP: ${otp}`);
  const html = `
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #f8faff; border-radius: 16px; overflow: hidden; border: 1px solid #e2e8f0;">
      <!-- Header -->
      <div style="background: linear-gradient(135deg, #4f46e5, #7c3aed); padding: 32px 36px; text-align: center;">
        <div style="font-size: 28px; font-weight: 800; color: #fff; letter-spacing: -0.02em;">🏠 HostelOS</div>
        <div style="color: rgba(255,255,255,0.8); font-size: 14px; margin-top: 6px;">Reset Your Password</div>
      </div>
      ...
      <div style="padding: 32px 36px;">
        <p style="margin: 0 0 12px; font-size: 16px; color: #0f172a; font-weight: 600;">Hello, ${name} 👋</p>
        <p style="margin: 0 0 24px; font-size: 14px; color: #64748b; line-height: 1.6;">
          Use the verification code below to proceed with your request. This code will expire in 10 minutes.
        </p>

        <!-- OTP box -->
        <div style="background: #fff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; text-align: center; margin-bottom: 24px;">
           <div style="font-size: 11px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 8px;">Verification Code</div>
           <div style="font-size: 32px; font-weight: 800; color: #4f46e5; letter-spacing: 0.2em; font-family: monospace;">${otp}</div>
        </div>
